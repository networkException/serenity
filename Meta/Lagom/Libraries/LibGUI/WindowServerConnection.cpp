...
void WindowServerConnection::async_did_finish_painting(i32 window_id, Vector<Gfx::IntRect> const&)
{
...
void WindowServerConnection::async_invalidate_rect(i32 window_id, Vector<Gfx::IntRect> const& rects, bool)
{
    auto window = Window::from_window_id(window_id);
...
void WindowServerConnection::async_set_fullscreen(i32 window_id, bool fullscreen)
{
    SDL_SetWindowFullscreen(SDLServer::the().window(window_id), fullscreen ? SDL_WINDOW_FULLSCREEN_DESKTOP : 0);
}

void WindowServerConnection::async_set_frameless(i32 window_id, bool frameless)
{
    SDL_SetWindowBordered(SDLServer::the().window(window_id), SDL_bool(!frameless));
}

void WindowServerConnection::async_set_maximized(i32 window_id, bool maximized)
{
    if (maximized)
...
void WindowServerConnection::async_set_window_opacity(i32 window_id, float opacity)
{
...
Gfx::IntRect WindowServerConnection::get_applet_rect_on_screen(i32)
{
