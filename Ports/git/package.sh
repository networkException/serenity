...
auth_type=sha256
configopts=("--target=${GELASSENHEIT_ARCH}-pc-serenity" "--with-lib=${GELASSENHEIT_INSTALL_ROOT}/usr/local" "CFLAGS=-DNO_IPV6" "LDFLAGS=-L${GELASSENHEIT_INSTALL_ROOT}/usr/local/lib")
depends=("zlib" "curl")
...
post_install() {
    run mkdir -p "${GELASSENHEIT_INSTALL_ROOT}/home/anon"
    run cp "../default_gitconfig" "${GELASSENHEIT_INSTALL_ROOT}/home/anon/.gitconfig"
}
