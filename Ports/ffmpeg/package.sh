...
auth_type="sha256"
installopts=("INSTALL_TOP=${GELASSENHEIT_INSTALL_ROOT}/usr/local")
configopts=("SRC_PATH=.")
...
install() {
    ...
    for lib in libavcodec libavdevice libavfilter libavformat libavutil; do
        ${CC} -shared -o ${GELASSENHEIT_INSTALL_ROOT}/usr/local/lib/${lib}.so -Wl,-soname,${lib}.so -Wl,--whole-archive ${GELASSENHEIT_INSTALL_ROOT}/usr/local/lib/${lib}.a -Wl,--no-whole-archive -liconv -ltiff -llzma -lbz2
        rm -f ${GELASSENHEIT_INSTALL_ROOT}/usr/local/lib/$lib.la
    done
}
