...
install() {
    ...
    ${CC} -shared -pthread -o ${GELASSENHEIT_INSTALL_ROOT}/usr/local/lib/libnpth.so -Wl,-soname,libnpth.so -Wl,--whole-archive ${GELASSENHEIT_INSTALL_ROOT}/usr/local/lib/libnpth.a -Wl,--no-whole-archive
    rm -f ${GELASSENHEIT_INSTALL_ROOT}/usr/local/lib/libnpth.la
}
