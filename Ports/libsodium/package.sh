...
install() {
    ...
    ${CC} -shared -pthread -o ${GELASSENHEIT_INSTALL_ROOT}/usr/local/lib/libsodium.so -Wl,-soname,libsodium.so -Wl,--whole-archive ${GELASSENHEIT_INSTALL_ROOT}/usr/local/lib/libsodium.a -Wl,--no-whole-archive
    rm -f ${GELASSENHEIT_INSTALL_ROOT}/usr/local/lib/libsodium.la
}
