...
static void fetch_internal_module_script_graph(JS::ModuleRequest module_request, EnvironmentSettingsObject& fetch_client_settings_object, StringView destination, EnvironmentSettingsObject& module_script_settings_object)
{

}

// https://html.spec.whatwg.org/multipage/webappapis.html#fetch-the-descendants-of-a-module-script
void JavaScriptModuleScript::fetch_descendants(EnvironmentSettingsObject& fetch_client_settings_object, StringView destination, HashTable<ModuleLocationTuple> visited_set, Function<void(JavaScriptModuleScript const*)> callback)
{
    ...
    for (auto const& request : module_requests) {
        ...
        // FIXME: Pass options.
        fetch_internal_module_script_graph(request, fetch_client_settings_object, destination, settings_object(), visited_set);
    }
}

// https://html.spec.whatwg.org/multipage/webappapis.html#fetch-the-descendants-of-and-link-a-module-script
void JavaScriptModuleScript::fetch_descendants_and_link(EnvironmentSettingsObject& fetch_client_settings_object, StringView destination, HashTable<ModuleMap::Key> visited_set, Function<void(JavaScriptModuleScript const*)> callback)
{
    // 1. Fetch the descendants of module script, given fetch client settings object, destination, and visited set.
    fetch_descendants(fetch_client_settings_object, destination, visited_set);
}
