    ...
    if (!vm) {
        ...
        vm->host_resolve_imported_module = [](JS::ScriptOrModule const& referencing_string_or_module, JS::ModuleRequest const& module_request) -> JS::ThrowCompletionOr<JS::NonnullGCPtr<JS::Module>> {
            ...
            if (!referencing_string_or_module.has<Empty>()) {
                // FIXME: 1. Let referencing script be referencingScriptOrModule.[[HostDefined]].
                // FIXME: 2. Set settings object to referencing script's settings object.
                // FIXME: 3. Set base URL to referencing script's base URL.
                // FIXME: 4. Assert: base URL is not null, as referencing script is a classic script or a JavaScript module script.
                TODO();
            }
            ...
            auto url = module_request.resolve_specifier(base_url);
