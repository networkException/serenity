    ...
    , m_custom_data(move(custom_data))
{
    ...
    host_load_imported_module = [this](Realm& realm, ImportedModuleReferrer const& referrer, ModuleRequest const& module_request, GCPtr<GraphLoadingState::HostDefined> load_state, ImportedModulePayload payload) -> void {
        dbgln("host_load_imported_module in VM");
        return load_imported_module(realm, referrer, module_request, load_state, move(payload));
    };
...
void VM::load_imported_module(Realm& realm, ImportedModuleReferrer const& referrer, ModuleRequest const& module_request, GCPtr<GraphLoadingState::HostDefined>, ImportedModulePayload payload)
{
    ...
    if (payload.has<NonnullGCPtr<PromiseCapability>>() && !m_allows_dynamic_imports) {
        ...
        finish_loading_imported_module(realm, referrer, module_request, payload, throw_completion<InternalError>(ErrorType::DynamicImportNotAllowed, module_request.module_specifier));
        return;
    }
    ...
    StringView base_filename = referrer.visit(
        [](NonnullGCPtr<Realm>) { return "."sv; },
        [](auto& script_or_module) { return script_or_module->filename(); });
    ...
    if (loaded_module_or_end != nullptr) {
        ...
        finish_loading_imported_module(realm, referrer, module_request, payload, *loaded_module_or_end->module);
        return;
    }
    ...
    if (file_or_error.is_error()) {
        finish_loading_imported_module(realm, referrer, module_request, payload, throw_completion<SyntaxError>(ErrorType::ModuleNotFound, module_request.module_specifier));
        return;
    }
    ...
    if (file_content_or_error.is_error()) {
        if (file_content_or_error.error().code() == ENOMEM) {
            finish_loading_imported_module(realm, referrer, module_request, payload, throw_completion<JS::InternalError>(error_message(::JS::VM::ErrorMessage::OutOfMemory)));
            return;
        }
    ...
    dbgln_if(JS_MODULE_DEBUG, "[JS MODULE] load_imported_module(...) parsed {}", filename);

    finish_loading_imported_module(realm, referrer, module_request, payload, module);
}
